...
tree_graph <- function(tree_tab = NULL, n = NULL, EY = NULL,
  ...
  A0_positions <- rep(paste0(
    "sloped, yshift=", A0_yshift, "pt, xshift=",
    A0_xshift, "pt, above"
  ), 2)
  L1_positions <- rep(paste0(
    "sloped, yshift=", L1_yshift, "pt, xshift=",
    L1_xshift, "pt, above, anchor=west"
  ), 4)
  A1_positions <- paste0(
    "sloped, yshift = ", rep(c("-", ""), 4),
  ...
  tree <- dagtex(
    ...
    # A0 numbers
    add_edge(1, 2,
      start_position = "east", end_position = "west", is_headless = TRUE,
    ...
    # L1 numbers
    add_edge(2, 2,
      start_position = "west", end_position = "north east", is_headless = TRUE,
      annotate = annotate_edge(ifelse(L1_annotate, L1a, ""),
        position = L1_positions[1]
      )
    ) %>%
    add_edge(2, 2,
      start_position = "west", end_position = "south east", is_headless = TRUE,
  ...
  for (i in 1:8) {
    tree <- tree %>%
      add_edge(l1_nodes[i], a1_nodes[i],
        start_position = a1_start_positions[i],
